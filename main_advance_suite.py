...
class ExtendedAutoTypingSoftware:
    def __init__(self, root):
        self.root = root
        self.root.title("Professional Auto Typing Suite")
        self.root.geometry("1100x850")  # Increased height for footer
        ...
        self.speed_var = tk.DoubleVar(value=150)
        self.accuracy_var = tk.DoubleVar(value=100)
        self.delay_var = tk.DoubleVar(value=3)
    ...
    def setup_sessions_tab(self):
        ...
        preview_frame.pack(fill=tk.BOTH, padx=10, pady=(0, 10), expand=False, height=150)
    ...
    def setup_footer(self):
        """Setup footer with copyright and info"""
        footer_frame = tk.Frame(self.root, bg='#252525', height=40)
        footer_frame.pack(side=tk.BOTTOM, fill=tk.X)
        footer_frame.pack_propagate(False)  # Prevent frame from shrinking

        # Left section - Copyright
        left_footer = tk.Frame(footer_frame, bg='#252525')
        left_footer.pack(side=tk.LEFT, fill=tk.Y, padx=15)

        copyright_label = tk.Label(
            left_footer,
            text="© 2024 Professional Auto Typing Suite v2.0 | All Rights Reserved",
            font=("Arial", 9),
            fg='#888',
            bg='#252525'
        )
        ...
        middle_footer = tk.Frame(footer_frame, bg='#252525')
        middle_footer.pack(side=tk.LEFT, fill=tk.Y, expand=True)

        self.system_info_label = tk.Label(
            middle_footer,
            text=f"Python {sys.version_info.major}.{sys.version_info.minor}.{sys.version_info.micro} | Platform: {platform.system()}",
            font=("Arial", 8),
            fg='#666',
            bg='#252525'
        )
        ...
        # Right section - Quick stats and links
        right_footer = tk.Frame(footer_frame, bg='#252525')
        right_footer.pack(side=tk.RIGHT, fill=tk.Y, padx=15)
        ...
        self.session_count_label.pack(side=tk.LEFT, padx=(0, 20))
        ...
        # Separator lines
        sep1 = tk.Frame(footer_frame, bg='#444', width=1, height=20)
        sep1.place(relx=0.33, rely=0.5, anchor='center')

        sep2 = tk.Frame(footer_frame, bg='#444', width=1, height=20)
        sep2.place(relx=0.66, rely=0.5, anchor='center')

        # Add hover effects
        copyright_label.bind("<Enter>", lambda e: copyright_label.config(fg='#aaa'))
        copyright_label.bind("<Leave>", lambda e: copyright_label.config(fg='#888'))

        # Add click functionality to copyright
        copyright_label.bind("<Button-1>", lambda e: self.show_about_dialog())

    def update_footer_info(self):
        """Update footer information periodically"""
        try:
            # Update session count
            session_count = len(self.sessions)
            self.session_count_label.config(text=f"Sessions: {session_count}")
            ...
            # If psutil is not available, show placeholder
        ...
        try:
            self.db_cursor.execute("SELECT COUNT(*) FROM sessions")
            self.db_status_label.config(text="Database: ✓", fg='#4CAF50')
        except:
            self.db_status_label.config(text="Database: ✗", fg='#f44336')

        # Update system info with time
        self.system_info_label.config(
            text=f"{current_date} {current_time} | Python {sys.version_info.major}.{sys.version_info.minor} | {platform.system()}"
        )

        # Schedule next update
        self.root.after(1000, self.update_footer_info)

    def show_about_dialog(self):
        """Show about dialog when copyright is clicked"""
        about_window = tk.Toplevel(self.root)
        about_window.title("About Professional Auto Typing Suite")
        ...
        # Logo/Title
        title_frame = tk.Frame(about_window, bg='#2b2b2b')
        ...
        tk.Label(
            title_frame,
            text="Professional Auto Typing Suite",
            font=("Arial", 18, "bold"),
            fg='white',
            bg='#2b2b2b'
        ).pack(pady=(10, 0))
        ...
        # Info section
        info_frame = tk.Frame(about_window, bg='#2b2b2b')
        info_frame.pack(fill=tk.BOTH, expand=True, padx=30, pady=20)

        info_text = """
Advanced automation software for efficient text typing and automation.

Features:
• Intelligent typing automation
...
Copyright © 2024 Auto Typing Software
All rights reserved.

This software is provided for legitimate automation purposes only.
...
""".format(
        ...
        tk.Button(
            button_frame,
            text="Website",
            command=lambda: webbrowser.open("https://www.autotypingsoftware.com"),
            bg='#2196F3',
            fg='white',
            relief=tk.FLAT,
            padx=30,
...
Professional Auto Typing Suite v2.0

1. GRANT OF LICENSE
This agreement grants you the right to use one copy of the Software on a single computer.

2. COPYRIGHT
The Software is owned by Auto Typing Software and is protected by copyright laws.
...
For full license terms, visit: https://www.autotypingsoftware.com/license
...
"""
    ...
    def save_session(self):
        ...
        if name:
            ...
            try:
                ...
                self.session_count_label.config(text=f"Sessions: {len(self.sessions)}")
            except Exception as e:
    ...
    def open_user_guide(self):
        ...
        webbrowser.open("https://github.com/nikjha/autotyping/wiki")

    def report_issue(self):
        """Open issue reporting"""
        webbrowser.open("https://github.com/nikjha/autotyping/issues")
